...
public:
  ...
  // 总之就是做了kAccessesPerGroupA次的AsyncCopy
  CUTLASS_DEVICE
    ...
    int &gemm_k_iterations)     ///< [in|out] number of threadblock mainloop iterations remaining
  {
    ...
    for (int stage = 0; stage < Base::kStages - 1; ++stage, --gemm_k_iterations) {
      ...
      // Async Copy for operand A
      // AsyncCopyIterationsPerStageA: PitchLinearShape::(kContiguous * kStrided)
      CUTLASS_PRAGMA_UNROLL
      for (int j = 0; j < Detail::AsyncCopyIterationsPerStageA; ++j) {
        typename IteratorA::AccessType *dst_ptr =
            reinterpret_cast<typename IteratorA::AccessType *>(
                this->smem_iterator_A_.get());
        // kAccessesPerVector 一次Access比如64B 一个Vector比如16B 那就是4个
        CUTLASS_PRAGMA_UNROLL
        for (int v = 0; v < IteratorA::kAccessesPerVector; ++v) {
          int const kSrcBytes =
  ...
  void mac_loop_iter(
    ...
    int &gemm_k_iterations)         ///< [in|out] number of threadblock mainloop iterations remaining
  {
    // kWarpGemmIterations: Number of warp-level GEMM oeprations
